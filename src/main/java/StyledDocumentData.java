        ...
        for (ParagraphData pd : paragraphData) {
            ...
            for (ContentData cd : pd.contentData) {
                ...
                for(AttributeData ad: cd.contentAttributeData) {
                    ...
                    switch (ad.type) {
                        case "name" -> contentStyle = theDocument.addStyle(ad.value, theDefault);
                        case "resolver" -> {
                            parentStyle = theDocument.getStyle(ad.value);
                            //s.addAttributes(parentStyle);
                            contentStyle.setResolveParent(parentStyle);
                        }
                        case "family" -> StyleConstants.setFontFamily(contentStyle, ad.value);
                        case "foreground" -> {
                        ...
                        case "icon" -> StyleConstants.setIcon(contentStyle, createImageIcon(ad.value));
                        case "italic" -> StyleConstants.setItalic(contentStyle, Boolean.parseBoolean(ad.value));
                        case "bold" -> StyleConstants.setBold(contentStyle, Boolean.parseBoolean(ad.value));
                        case "underline" -> StyleConstants.setUnderline(contentStyle, Boolean.parseBoolean(ad.value));
                        case "superscript" -> StyleConstants.setSuperscript(contentStyle, Boolean.parseBoolean(ad.value));
                        case "subscript" -> StyleConstants.setSubscript(contentStyle, Boolean.parseBoolean(ad.value));
